...
void MESI_Controller::queueBusCommand(unsigned int command, unsigned long long payload, unsigned int targetAdx) {
...
void MESI_Controller::queueMaxPriorityBusCommand(unsigned int command, unsigned long long payload, unsigned int targetAdx) {
    BusRequest* r    = new BusRequest;
    r->commandCode   = command;
    r->payload       = cache->getLineAlignedAddress(payload);
    r->targetAddress = targetAdx;
